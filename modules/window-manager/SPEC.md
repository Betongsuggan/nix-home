...
- Keyboard layout: all backends default to US Colemak (Hyprland: `input:kb_layout = us`, `kb_variant = colemak`). The hyprland backend additionally pins input-remapper's virtual keyboards (`input-remapper-keyboard`, `input-remapper-keyboard-+-mouse`) to plain US via `device` blocks: `my.input-remapper` mapping symbols are evdev key names (`KEY_S` = physical S key), so reading that device through Colemak turned a WASD thumbstick mapping into "wars". Per-device settings override `input:` and are not touched by the layout toggle; the blocks are inert when no such device exists. The other backends don't carry this override yet (sway could via `input "input-remapper keyboard" xkb_layout us`; niri has no per-device xkb).
