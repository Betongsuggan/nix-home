...
let
  ...
  buildDunstifyCmd =
    ...
    let
      ...
      hintFlags = concatStringsSep " " (mapAttrsToList (k: v: "-h ${k}:${toString v}") hints);

      # Stack tag for replacing notifications
      stackTagFlag = optionalString (replaceTag != null) "-h string:x-dunst-stack-tag:${replaceTag}";

      # Progress bar value (rendered as a bar by dunst)
      progressFlag = optionalString (progress != null) "-h int:value:${toString progress}";

      summaryArg = "\"${summary}\"";
      bodyArg = optionalString (body != "") "\"${body}\"";

    in
    "${pkgs.dunst}/bin/dunstify ${urgencyFlag} ${iconFlag} ${appNameFlag} ${timeoutFlag} ${hintFlags} ${stackTagFlag} ${progressFlag} ${summaryArg} ${bodyArg}";
  ...
  buildNotifySendCmd =
    ...
    in
    "${pkgs.libnotify}/bin/notify-send ${urgencyFlag} ${iconFlag} ${appNameFlag} ${timeoutFlag} ${hintFlags} ${progressFlag} ${summaryArg} ${bodyArg}";
