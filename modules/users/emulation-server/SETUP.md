# Emulation Server User Module

User-level module that creates the emulation directory structure. Intended
for the user account on the server machine (the same user that Syncthing
and Samba run as).

This module is separate from the system module because it uses Home Manager's
`home.activation` to create directories owned by the user, while the system
module handles service configuration (Syncthing, Samba, WireGuard).

## Usage

In the server user's config (e.g. `user-gamer.nix`):

```nix
emulation-server.enable = true;
```

See `modules/system/emulation-server/SETUP.md` for full server setup instructions.

## Customizing systems and emulators
...
emulation-server = {
  ...
  # Only create ROM dirs for systems you actually have
  systems = [ "snes" "nes" "gba" "nds" "psx" "psp" ];

  # Only create save dirs for emulators you use
...
snes, nes, gb, gbc, gba, n64, nds, psx, ps2, psp, megadrive, mastersystem,
gamecube, wii, dreamcast, saturn, arcade
...
Android devices don't use NixOS modules but can still connect to the server
for save sync and ROM access.

### Save sync via Syncthing

1. Install **Syncthing** from F-Droid or Play Store
2. Open Syncthing and note your device ID (shown on the main screen)
...
5. On Android Syncthing, add the server as a device using the server's device ID
   (find it at http://server-ip:8384 -> Actions -> Show ID)
6. Accept the `emulation-saves` folder share when it appears
7. Set the folder path to a convenient location, e.g. `/storage/emulated/0/emulation/saves`
...
For RetroArch on Android, you can add the SMB path as a content directory.
...
2. Create a new tunnel with this template:
   ...
   Address = 10.100.0.3/24   # Unique IP, different from other clients
   DNS = 1.1.1.1

   [Peer]
   PublicKey = <server's public key from /etc/wireguard/public.key>
...
| `systems` | list of string | (17 systems) | ROM subdirectories to create |
