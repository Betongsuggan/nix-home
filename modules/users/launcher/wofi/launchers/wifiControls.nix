  ...
  wifiControl = pkgs.writeShellScriptBin "wifi-control" ''
    ...
    if [ "$chosen_network" = "" ]; then
    	...
    	if [[ $(echo "$saved_connections" | grep -w "$chosen_id") = "$chosen_id" ]]; then
    		nmcli connection up id "$chosen_id" | grep "successfully" && ${notifyWifiConnected}
    	else
    		if [[ "$chosen_network" =~ "" ]]; then
    			wifi_password=$(wofi --dmenu --password --prompt "Password: " )
    		fi
    		nmcli device wifi connect "$chosen_id" password "$wifi_password" | grep "successfully" && ${notifyWifiConnected}
        fi
    fi
  '';
in
