...
- Wake-on-LAN relays: every host gets a `wake-<host>` command for each registry host whose `wol.relay` names it (e.g. controller: `wake-desktop`, island-pi: `wake-island-stationary`), since the magic packet has to be sent on the target's LAN.
