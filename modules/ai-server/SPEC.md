...
- Mobile browsers require **HTTPS** for `getUserMedia`. The simplest path on desktop is `tailscale serve --bg --https=443 http://localhost:8081`, which provisions an ACME cert and routes `https://desktop.<tailnet>.ts.net` → Open WebUI. The mapping persists in tailscale state across reboots.
