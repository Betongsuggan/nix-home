...
- Wine for launchers outside Steam (Lutris, Bottles) is nixpkgs' `wineWow64Packages.staging`: the staging patchset in WoW64 mode, where 32-bit programs run inside the 64-bit build. Hydra builds this variant, not `wineWowPackages` (which would compile locally). nix-gaming's wine-tkg was considered but isn't in any binary cache, so every nix-gaming update would compile Wine locally; Steam games use Proton-GE either way.
