...
| utils.autoScreenRotation | bool | false | Rotate the built-in panel (`my.window-manager.touchOutput`, default eDP-1) with the accelerometer while in tablet mode, on Hyprland or niri. A `tablet-mode-watch` user service follows the switch via libinput and starts/stops `auto-screen-rotation`; the user is added to the `input` group for that. |
...
- `battery-notifier` (Mod+B) shows percentage, current battery power draw (W, from upower) and time left/to full.
