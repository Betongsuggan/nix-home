...
- Hyprland compositor on both users, with outputs, workspace placement and idle behavior declared once in `session.nix` (imported by both users): the PC monitor DP-2 (3440x1440@240, HDR, VRR) holds workspaces 1-9; the TV on HDMI-A-2 (4K HDR, VRR, right of DP-2) always shows as an output, because HDMI stays connected while the TV is off, but only holds its own workspace 11 until a window is moved there (Mod+Ctrl+Shift+H/L, focus with Mod+Ctrl+H/L); DP-1 (the forced headless connector, below) is disabled in Hyprland. VRR is on for both screens and both users: without it the top of DP-2 flickered white in HDR
...
- Logitech G13 gaming keypad with declarative key remapping via input-remapper (auto-applies on connect; `g13.nix`). M1 is for WoW wall climbing: a jump-then-forward cycle (Space, W 10 ms later, 50 ms, 30 ms gap) repeated while held; M2 the same strafing right (+ D) (macros)
...
- Auto-suspend after 30 min of session-idle via `services.logind.settings.Login.IdleAction = "suspend"`, the only idle suspend on this host (`my.window-manager.idle.suspendAfter = null` in `session.nix`: the compositor doesn't see gamepad input, so its own 15 min suspend would cut a controller-only stream short). A running stream additionally holds a `systemd-inhibit` idle/sleep lock. Local input, Sunshine streaming and SSH sessions all keep it awake via the normal logind idle-hint path; the rare gap is a long-running AI generation interrupted mid-stream — recovered by a client retry through controller's wake-proxy.
