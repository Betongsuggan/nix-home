...
- Tailnet subnet router: advertises the home LAN `192.168.50.0/24` so tailnet peers (which run `--accept-routes` via the `tailnet` module) can reach LAN devices from anywhere. LAN devices see incoming tailnet traffic SNATed to controller's LAN IP, so no return routes are needed on the router. Activation on an already-registered node is manual (up-flags apply at registration only): `sudo tailscale set --advertise-routes=192.168.50.0/24`, then approve with `sudo headscale nodes approve-routes --identifier <id> --routes 192.168.50.0/24`. Named LAN devices get `<name>.home.rydback.net` A records via `extraDnsRecords` (currently `router.home.rydback.net` → 192.168.50.1); only DHCP-reserved/static addresses should be added there.
